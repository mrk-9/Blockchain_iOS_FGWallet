...
class WelcomeViewController : UIViewController, ContentBoxPresenter {
    ...
    private func setBodyText() {
        ...
        let attributedString = NSMutableAttributedString.init(string: S.Welcome.body)    //chuch0805
        ...
        let range = NSString(string: S.Welcome.body).range(of: "(?)")                                       //chuch0805
//        let nsRange = bodyText.lineRange(for: range!)
        attributedString.replaceCharacters(in: range, with: NSAttributedString(attachment: icon))           //chuch0805
