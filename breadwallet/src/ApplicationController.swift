...
class ApplicationController : Subscriber, Trackable {
    ...
    var walletManagerReinited = false
   // Bool walletManagerReinited = false;
    ...
    private func didInitWalletManager() {
       ...
       // guard let wallet = walletManager?.wallet else { return }
        // Sender(walletManager: walletManager, kvStore: kvStore, store: store        //globalAPI?.walletManager = walletManager,
            
        globalAPI = API(walletManager: walletManager,store: store);       // globalAPI = API(wallet: wallet, store: store)
        // myAPI.walletManager = walletManager
        ...
        if UIApplication.shared.applicationState != .background {
            ...
            else
            {
                ...
                let pin1 = UserDefaults.standard.value(forKey: "PIN") as? String
                    ...
                    DispatchQueue.walletQueue.async
                        {
                            ...
                            DispatchQueue.main.asyncAfter(deadline: .now() + 2.0, execute:
                                {
                                    if(walletManager.wipeWallet(pin: "forceWipe")==true && self.walletManagerReinited == false)
                                    {
                                        self.store.trigger(name: .reinitWalletManager({}))
                                        self.walletManagerReinited = true;
    ...
    private func setupRootViewController()
    {
     ...
     //FG
       // globalAPI = API(store: store, didSelectTransaction: didSelectTransaction)
        //myAPI = API
        
        let menuLeft = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "MenuLeft") as! MenuLeft
        
        
        if let pin = UserDefaults.standard.value(forKey: "PIN") as? String, !pin.isEmpty {
            let loginVC = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "LoginVC") as! LoginVC
            let sliderVC = SWRevealViewController(rearViewController: menuLeft, frontViewController: UINavigationController.init(rootViewController: loginVC))
            ...
            let sliderVC = SWRevealViewController(rearViewController: menuLeft, frontViewController: UINavigationController.init(rootViewController: mainVC))
           // sliderVC.str = "9"
            window.rootViewController = sliderVC
